...
Engine::Engine(
  ...
  fxl::Closure on_session_metrics_change_callback =
      std::bind(&Engine::OnSessionMetricsDidChange, this);
  ...
  shell::Shell::CreateCallback<shell::PlatformView> on_create_platform_view =
      fxl::MakeCopyable([parent_environment_service_provider =
        ...
        return std::make_unique<flutter::PlatformView>(
            ...
            std::move(scene_manager),                        // scene manager
...
void Engine::OnSessionMetricsDidChange() {
  ...
  shell_->GetTaskRunners().GetUITaskRunner()->PostTask(
      [engine = shell_->GetEngine()]() {
        if (engine) {
          engine->ScheduleFrame(true /* regenerate layer tree */);
        }
      });
}
